...
const ChatbotWidget = ({ jdText, cvFileName }: ChatbotWidgetProps) => {
  ...
  const handleSend = () => {
    ...
    // Mock response - will be replaced with real AI
    setTimeout(() => {
      const contextInfo = cvFileName ? `\n\n(Tôi đang xem CV: ${cvFileName}${jdText ? " và JD bạn đã nhập" : ""})` : "";
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content: `Cảm ơn bạn đã hỏi! Đây là phiên bản demo, chức năng AI chatbot sẽ được kết nối sau khi bật Lovable Cloud.${contextInfo}\n\nTôi sẽ có thể giúp bạn:\n• Viết lại bullet point theo phương pháp STAR\n• Thêm action verbs mạnh\n• Định lượng kết quả công việc`,
        },
      ]);
      setIsTyping(false);
    }, 1500);
